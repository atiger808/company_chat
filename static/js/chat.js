...
// 发送消息（支持文件消息）
    ...
    renderChatRooms() {
        ...
        sortedRooms.forEach(room => {
            ...
            console.log('render room: ', room, ' last: ,', room.last_message)
...
// 获取文件类型
    ...
    getFileMessageContent(message_type) {
        ...
        if (type.includes('voice')) return '[音频]';
        return '';
    }
...
// 更新群聊信息
    ...
    async deleteGroupChat(roomId) {
        const confirmed = await this.showConfirmDialog(
            '删除群聊',
            '确定要删除整个群聊吗？<br><small style="color: var(--text-light);">此操作不可恢复！</small>',
            'danger'
        );
        if (!confirmed) return;
