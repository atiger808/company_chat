...
class CloudApp {
    ...
    // 🔧 创建协作文档
    async createCollabDocument() {
        const name = document.getElementById('newCollabDocName')?.value.trim();
        const docType = document.getElementById('newCollabDocType')?.value;
        const folderId = document.getElementById('newCollabDocFolder')?.value || null;
        const description = document.getElementById('newCollabDocDesc')?.value || '';
        const defaultPermission = document.getElementById('defaultCollabPermission')?.value || 'write';
        let initialCollaborators = []

        if (this.selectedCollabUsers.size > 0) {
            for (const userId of this.selectedCollabUsers) {
                initialCollaborators.push({'user_id': userId, 'permission': defaultPermission})
            }
        }
        ...
        try {
            ...
            const result = await response.json();
            console.log('协作文档创建成功:', result);

            // 🔧 如果有初始协作者，添加他们
            // if (this.selectedCollabUsers.size > 0) {
            //     for (const userId of this.selectedCollabUsers) {
            //         await fetch(`/api/cloud/documents/${result.file_id}/add_collaborator/`, {
            //             method: 'POST',
            //             headers: {
            //                 'Content-Type': 'application/json',
            //                 ...TokenManager.getHeaders()
            ...
            this.showSuccess('创建成功', '协作文档已创建');
            this.closeModal('newCollabDocModal');

            // 重置表单
            document.getElementById('newCollabDocName').value = '';
    ...
    // 🔧 搜索协作用户（用于新建文档时）
    async searchCollabUsers(keyword) {
        if (!keyword.trim()) {
            document.getElementById('collabUserResults').innerHTML = '';
        ...
        try {
            ...
            const filtered = users.filter(u =>
                !this.selectedCollabUsers.has(u.id.toString()) &&
                u.id !== this.currentUser?.id
            );

            this.renderCollabUserResults(filtered);
    ...
    // 🔧 渲染协作用户搜索结果
    renderCollabUserResults(users) {
        const container = document.getElementById('collabUserResults');
        if (users.length === 0) {
            container.innerHTML = '<div style="padding:10px;color:#999;">未找到用户</div>';
        ...
        users.forEach(user => {
            html += `
            <div class="user-search-item" onclick="cloudApp.addCollabUser('${user.id}', '${this.escapeHtml(user.real_name || user.username)}')">
                ...
                <div style="flex:1;">
                    ...
                    <div style="font-size:12px;color:#999;">${user.department_info?.name || ''}</div>
                </div>
                <i class="fas fa-plus-circle" style="color:#409EFF;"></i>
            </div>
        `;
        });
        container.innerHTML = html;
    ...
    // 🔧 添加协作用户到选中列表
    addCollabUser(userId, userName) {
        this.selectedCollabUsers.add(userId);
        this.updateSelectedCollabsDisplay();
        document.getElementById('collabUserResults').classList.remove('show');
        document.getElementById('collabUserSearch').value = '';
        this.showSuccess('已添加', `${userName} 已添加到协作者列表`);
    }

    // 🔧 更新已选协作者显示
    updateSelectedCollabsDisplay() {
        const container = document.getElementById('selectedCollabs');
        if (!container) return;

        let html = '';
        this.selectedCollabUsers.forEach(userId => {
            // 这里需要缓存用户信息，简化处理
            html += `
            <span class="collab-tag">
                <span>用户 #${userId}</span>
                <i class="fas fa-times remove-collab" onclick="cloudApp.removeSelectedCollab('${userId}')"></i>
            </span>
        `;
        });
        container.innerHTML = html || '<small class="text-muted">暂无选中的协作者</small>';
    }

    // 🔧 移除已选协作用户
    removeSelectedCollab(userId) {
        this.selectedCollabUsers.delete(userId);
        this.updateSelectedCollabsDisplay();
    }

    // 🔧 打开新建协作文档模态框
    openNewCollabDocModal() {
        // 加载文件夹列表
        this.loadCollabDocFolders();
        // 清空表单
        document.getElementById('newCollabDocName').value = '';
        document.getElementById('newCollabDocDesc').value = '';
        this.selectedCollabUsers.clear();
        this.updateSelectedCollabsDisplay();
        // 显示模态框
        this.openModal('newCollabDocModal');
    }

    // 🔧 加载协作文档文件夹选项
    async loadCollabDocFolders() {
        try {
            const response = await fetch('/api/cloud/folders/tree/', {
                headers: TokenManager.getHeaders()
            });
            ...
            const data = await response.json();
            const select = document.getElementById('newCollabDocFolder');
            if (!select) return;
            ...
            const addFolderOptions = (folders, level = 0) => {
                folders.forEach(folder => {
                    ...
                    html += `<option value="${folder.id}">${indent}📁 ${this.escapeHtml(folder.name)}</option>`;
    ...
    async confirmFileSelect() {
        ...
        try {
            ...
            // 🔧 关键：调用后端接口从文件创建协作文档
            const response = await fetch('/api/cloud/documents/create-from-file/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    ...TokenManager.getHeaders()
                ...
                body: JSON.stringify({
                    ...
                    // 如需重命名可添加: title: this.selectedFileName + ' - 副本'
                })
            });
    ...
    setupEventListeners() {
        ...
        if (collabUserSearch) {
            collabUserSearch.addEventListener('input', Utils.debounce((e) => {
                this.searchCollabUsers(e.target.value);
            }, 300));
