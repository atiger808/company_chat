...
class DocumentEditorApp {
    ...
    async init() {
        try {
            ...
            // 🔧 7. 优化：首次打开文档时，仅调用一次更新协同状态和加载协作者（移除持续心跳）
    ...
    handleCollabMessage(data) {
        ...
        switch (type) {
            ...
            case 'user_joined':
                ...
                this.loadCollaborators(); // 🔧 关键：收到加入消息后，拉取最新列表确保数据绝对同步
                break;

            case 'user_left':
                this.handleUserLeft(payload);
                this.loadCollaborators(); // 🔧 关键：收到离开消息后，拉取最新列表确保数据绝对同步
                break;

            case 'collab_status_update':
                // 🔧 实时更新协作者状态（editing/viewing/closed）
                this.updateCollaboratorStatus(payload);
                this.loadCollaborators(); // 🔧 关键：状态变更（如编辑/查看切换）后，拉取最新列表
                break;
            ...
            case 'heartbeat':
                // 🔧 心跳保活消息，无需处理（完全依赖 WebSocket 连接状态）
                break;
    ...
    async updateCollaborationStatus(status) {
        try {
            const response = await fetch(`/api/cloud/documents/${this.fileId}/collaboration/status/`, {
                ...
                keepalive: true // 🔧 关键修复：允许在页面卸载时继续发送请求，防止浏览器中断
            });
...
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') {
         console.log('页面隐藏，停止 HTTP 状态上报，依赖 WebSocket 保持连接');
    } else {
        try {
            // 页面恢复时，仅重新拉取一次协作者列表，确保 UI 状态同步
            console.log('页面恢复时同步协作者列表...');
            if (window.editorApp) {
                window.editorApp.loadCollaborators();
            }
        } catch (error) {
...
window.addEventListener('beforeunload', async (event) => {
    if (window.editorApp) {
        // 同步上报离开状态 (得益于 fetch 的 keepalive: true，这里能确保发送成功)
        try {
