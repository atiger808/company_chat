...
class CloudSystemConfigManager:
    """系统配置管理器 - 确保配置实际生效"""

    # 🔧 关键修复：使用与 settings.py 一致的缓存键前缀
    CACHE_PREFIX = 'cloud_config:'  # ✅ 添加前缀避免冲突
    CACHE_TIMEOUT = 300  # 5 分钟缓存
    ...
    def get_config(cls, key, default=None):
        ...
        try:
            ...
            predefined = CloudSystemSettingsViewSet.PREDEFINED_CONFIGS.get(key, {})
            default_value = predefined.get('default', default)
    ...
    def set_config(cls, key, value, user=None):
        ...
        config, created = CloudSystemConfig.objects.update_or_create(
            ...
            defaults={
                ...
                'value': str(value),
                'value_type': value_type,
                'category': predefined.get('category', 'system'),
                'updated_by': user
            }
        )
