...
> 基于 **Django + Channels + Celery + Redis** 的企业级即时通讯与协同办公平台，支持电脑浏览器、手机浏览器与 PWA 桌面 App。

## 功能模块

| 模块 | 说明 |
| --- | --- |
| 即时聊天 | 私聊/群聊、语音/视频通话、语音消息、文件/图片发送、消息引用/撤回/转发、工作总结/审批/任务卡片 |
| 考勤打卡 | 上下班打卡、白班/夜班、补卡、个人/部门/子公司/全局配置、定位记录、月度统计 |
| OA 审批 | 请假/加班/报销/出差/采购等动态审批类型，顺序/会签流程、阈值审批、票据回传、物资需求单/领用单、待我审批/我已审批、详情上一条/下一条连续审批 |
| 每日工作总结 | 员工上传工作数据 + 当日总结，按职位生成提示词，Celery 异步流式调用**火山方舟大模型**分析（deepseek 式打字输出）；部门负责人看本部门、超管看全员；范围批量分析、导出（Markdown/Word/PDF→本地或网盘）、私聊分享、打印预览 |
| 普惠补贴 | 发票申领/OCR 识别/二维码验证/百度验真、财务核验、财务支付、钱包/提现、三级配置 |
| 企业网盘 | 文件上传（分片/断点续传）/下载/分享、OnlyOffice 在线协作编辑、共享文件夹、回收站、操作权限（打印/下载/公开分享） |
| 工作日历 | 跨模块工作汇总、每日通知、时间线/审批效率图表、**全员每日总结完成情况图表（超管）** |
| 任务中心 | 任务创建/分配、看板拖拽、任务卡片分享到聊天 |
| 组织架构 | 企业/子公司/部门树、成员管理、负责人/副负责人、汇报关系（多企业隔离） |
| 工作通知 | 审批/考勤/任务/协作/每日汇总/总结分享，WebSocket 实时 + 系统推送 |
| 管理控制台 | 用户/聊天室管理、系统设置、操作日志/登录日志、数据统计、企业水印设置 |
| 企业水印 | 显性+隐性水印逐页生效、打印水印、打印操作留痕 |
...
| 大模型 | 火山方舟（豆包）OpenAI 兼容流式接口（`ARK_API_KEY` / `ARK_MODEL`） |
| 文档协作 | OnlyOffice |
| 前端 | 原生 JavaScript + Font Awesome 6 + ECharts/Chart.js |
| 加密 | AES + 国密 SM4；认证 JWT |

> 更详细的模块/文件/接口说明见 [`docs/企业协同办公平台 · 项目结构文档.md`](docs/企业协同办公平台%20·%20项目结构文档.md)，员工使用说明见 [`docs/企业协同办公平台 · 员工使用手册.md`](docs/企业协同办公平台%20·%20员工使用手册.md)。
