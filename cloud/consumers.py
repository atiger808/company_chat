...
class DocumentCollabConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # 最终检查认证
        if not self.user or self.user.is_anonymous:
            logger.warning(f'认证失败: file_id={self.file_id}')
            await self.close(code=4001)
            return

        # 检查访问权限
        if not await self._check_access_permission():
            logger.warning(f'权限不足: user={self.user.username}, file_id={self.file_id}')
        ...
        await CollabMessageBroadcaster.abroadcast_collab_message(
            ...
            data={
                'user': {
                    ...
                    'avatar': await self._get_user_avatar(self.user),
    ...
    async def _get_user_avatar(self, user):
        """获取用户头像"""
        if hasattr(user, 'get_avatar_url'):
            return user.get_avatar_url()
        return getattr(user, 'avatar_url', '/static/images/default-avatar.png')
